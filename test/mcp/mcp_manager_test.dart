...
void main() {
  ...
  setUp(() { manager = McpManager(); });

  group('McpManager', () {
    test('getAllTools merges tools from all servers', () {
      manager.addServerForTesting('kan',
      ...
      expect(tools.map((t) => t.name),
          containsAll(<String>['kan_list_boards', 'kan_create_card', 'outline_search']));
    });

    test('callTool routes to correct server', () async {
      manager.addServerForTesting('kan',
          McpToolInfo(name: 'kan_search', description: 'Search',
              handler: (a) async => '{"results": ["task-1"]}'));
      manager.addServerForTesting('outline',
          McpToolInfo(name: 'outline_search', description: 'Search',
              handler: (a) async => '{"results": ["doc-1"]}'));
      expect(await manager.callTool('kan_search', <String, dynamic>{}), contains('task-1'));
      expect(await manager.callTool('outline_search', <String, dynamic>{}), contains('doc-1'));
    });

    test('callTool throws for unknown tool', () {
      manager.addServerForTesting('kan',
          McpToolInfo(name: 'kan_search', description: 'Search'));
      expect(() => manager.callTool('nonexistent', <String, dynamic>{}), throwsA(isA<Exception>()));
    });

    test('getServerNames returns all servers', () {
      manager.addServerForTesting('kan', McpToolInfo(name: 'k', description: 's'));
      manager.addServerForTesting('outline', McpToolInfo(name: 'o', description: 's'));
  ...
  group('ToolRegistry + McpManager', () {
    test('merges MCP and custom tools', () {
      ...
      reg.registerCustomTool(CustomToolDef(
        name: 'sprint_info', description: 'Sprint info',
        inputSchema: const <String, dynamic>{'type': 'object', 'properties': <String, dynamic>{}},
        handler: (a) async => 'Sprint 5',
      ));
      manager.addServerForTesting('kan', McpToolInfo(name: 'kan_search', description: 'Search'));
      reg.setMcpManager(manager);
      final tools = reg.getAllToolDefinitions();
      expect(tools, hasLength(2));
      expect(tools.map((t) => t.name), containsAll(<String>['sprint_info', 'kan_search']));
    });
    ...
    test('executeTool routes custom tools first', () async {
      ...
      expect(await reg.executeTool('my_tool', <String, dynamic>{}), equals('custom result'));
    });
  });
}

...
mixin BoardConfigQueries {
  ...
  /// Returns the default board config for [groupId], or `null`.
  DefaultBoardConfigRecord? getDefaultBoardConfig(String groupId) {
    final rows = db.handle.select(
      'SELECT * FROM default_board_config WHERE group_id = ?',
      [groupId],
    );
  ...
  void upsertDefaultBoardConfig({
    ...
    db.handle.execute(
      ...
      '(group_id, board_public_id, list_public_id, board_name, list_name) '
      'VALUES (?, ?, ?, ?, ?) '
      ...
      [groupId, boardPublicId, listPublicId, boardName, listName],
    );
  }
